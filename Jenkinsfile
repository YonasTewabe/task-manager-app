pipeline {
    ...
    environment {
        ...
        FRONTEND_TARGET = '/var/www/task-manager'
        PM2_PROCESS_NAME = 'task-manager-backend'
    }
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'toAccessGithub', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                    sshagent(credentials: ['pepadmin']) {
                        script {
                            def repoUrl = "https://${GIT_USER}:${GIT_PASS}@github.com/IE-Network-Solutions/task-manager.git"
        ...
        stage('Copy env') {
            steps {
                sshagent(credentials: ['pepadmin']) {
                    sh """
                        ssh -o StrictHostKeyChecking=no ${REMOTE_USER}@${SERVER_IP} '
                            sudo cp ~/env/.task-front ${FRONTEND_DIR}/.env &&
                            sudo cp ~/env/.task-back ${BACKEND_DIR}/.env
                        '
                    """
                }
            }
