...
const DUMMY_PASSWORD_HASH =
  "$2b$10$e6.Gnmu4w0XyrJ5M8Wf4v.Zw3nD7kjm8v7R2M7.WQ8ixH9czA2QxK";
...
function buildAppUrl(req: Request, pathname = "") {
...
function enforceBodySizeLimits(
  req: Request,
  ...
  for (const [field, maxLength] of Object.entries(fields)) {
    ...
    const length = String(body[field]).length;
    if (length > maxLength) {
...
export async function registerHandler(req: Request, res: Response) {
  const body = asObjectRecord(req.body);
  const validation = requireFields(body, ["name", "email", "password"]);
  if (!validation.ok) {
    return res.status(400).json({ error: `Missing fields: ${validation.missing.join(", ")}` });
  }
  const sizeError = enforceBodySizeLimits(req, { name: 120, email: 320, password: 128 });
  ...
  if (!isAcceptablePassword(password)) {
    return res.status(400).json({ error: "Password is required and must be 1-128 characters." });
  }
...
export async function loginHandler(req: Request, res: Response) {
  ...
  const sizeError = enforceBodySizeLimits(req, { email: 320, password: 128 });
  if (sizeError) return res.status(400).json({ error: sizeError });
  const email = normalizeEmail(body.email);
  const password = asString(body.password);
  if (!isValidEmail(email)) return res.status(400).json({ error: "Invalid email format." });
  const loginKey = buildAttemptKey(req, email);
  const loginAttempt = consumeAttempt(loginAttemptStore, loginKey, LOGIN_WINDOW_MS, LOGIN_MAX_ATTEMPTS);
  ...
  if (!row.isActive) return res.status(403).json({ error: "This account is disabled." });
...
export async function forgotPasswordHandler(req: Request, res: Response) {
  ...
  const attempt = consumeAttempt(forgotAttemptStore, attemptKey, FORGOT_WINDOW_MS, FORGOT_MAX_ATTEMPTS);
  if (attempt.blocked) {
    return res.status(429).json({
  ...
  const resetUrl = buildAppUrl(req, `/reset-password?token=${encodeURIComponent(rawToken)}`);
...
export async function resetPasswordHandler(req: Request, res: Response) {
  const body = asObjectRecord(req.body);
  const validation = requireFields(body, ["token", "password"]);
  if (!validation.ok) {
    return res.status(400).json({ error: `Missing fields: ${validation.missing.join(", ")}` });
  }
  const token = asString(body.token).trim();
  const attemptKey = buildAttemptKey(req, token ? `reset:${token.slice(0, 8)}` : "reset");
  const attempt = consumeAttempt(resetAttemptStore, attemptKey, RESET_WINDOW_MS, RESET_MAX_ATTEMPTS);
  if (attempt.blocked) {
    return res.status(429).json({
...
export async function changePasswordHandler(req: Request, res: Response) {
  const body = asObjectRecord(req.body);
  const validation = requireFields(body, ["newPassword"]);
  if (!validation.ok) {
    return res.status(400).json({ error: `Missing fields: ${validation.missing.join(", ")}` });
  }
  ...
  if (!user.mustChangePassword) {
    ...
    if (!isCurrentValid) return res.status(401).json({ error: "Current password is invalid." });
  }
