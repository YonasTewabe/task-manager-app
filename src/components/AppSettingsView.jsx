...
export default function AppSettingsView({ canManage = false, onNotify }) {
  ...
  return (
    <section className="grid gap-[0.9rem] rounded-lg border border-[#dfe1e6] bg-white p-[0.8rem]">
      <div className="flex items-center justify-between gap-3">
        ...
        {canManage ? (
          !isEditMode ? (
            <button
              type="button"
              className="rounded-[8px] border border-[#d0d7e2] bg-[#f7f8fa] px-3 py-2 text-[0.82rem] font-medium text-[#42526e] hover:border-[#a8b3c5] hover:bg-white/65 hover:text-[#2f3d55]"
      ...
      {showEnableEditModal ? (
        <Modal
          open={showEnableEditModal}
          cardClassName="max-w-[460px]"
          onOpenChange={(open) => setShowEnableEditModal(open)}
        >
          <div className="grid gap-[0.7rem]">
            <h3>Enable settings editing?</h3>
            <p className="text-[0.9rem] text-[#42526e]">
              You are about to edit global GitHub integration credentials.
              Continue only if you intend to change system-level configuration.
            </p>
            <div className="flex justify-end gap-2">
              <button
                type="button"
                className="border border-[#dfe1e6] bg-transparent text-[#42526e] hover:bg-[#f4f5f7]"
                onClick={() => setShowEnableEditModal(false)}
              >
                Cancel
              </button>
              <button
      ...
      {showSaveConfirmModal ? (
        <Modal
          ...
          cardClassName="max-w-[460px]"
          onOpenChange={(open) => setShowSaveConfirmModal(open)}
        >
          <div className="grid gap-[0.7rem]">
            <h3>Confirm save settings</h3>
            <p className="text-[0.9rem] text-[#42526e]">
              Save changes to system-level GitHub integration credentials?
            </p>
            <div className="flex justify-end gap-2">
              <button
                type="button"
