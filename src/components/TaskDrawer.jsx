...
export default function TaskDrawer({
  ...
  const normalizeAcceptanceCriteria = useCallback((value) => {
    ...
    return list
      .map((item, index) => {
        ...
        if (!text) return null;
        return {
          // Keep IDs deterministic so autosave diffing does not detect false changes.
          id: String(item?.id || "").trim() || `ac-${index}-${text}`,
          text,
          done: item?.done === true,
        };
      })
  ...
  const lastSavedTaskIdRef = useRef(null);
  ...
  const allowedStatusKeys = useMemo(() => {
    ...
    const nextKeys = workflowTransitions
      .filter((transition) => {
        if (String(transition?.from || "") !== currentStatus) return false;
  ...
  }, [stages, task?.status, workflowTransitions, currentUserId, actorGroupIds]);
  ...
  const toComparablePatchJson = useCallback((patch) => {
    const normalizedDescription = toEditorRichText(patch?.description || "")
      .replace(/>\s+</g, "><")
      .trim();
    ...
    return JSON.stringify({
      ...
      dueDate: normalizeDateForInput(patch?.dueDate) || null,
      assigneeId: patch?.assigneeId ? String(patch.assigneeId) : null,
  ...
  const submitPatch = async () => {
    ...
    const patchJson = toComparablePatchJson(patch);
    lastSavedPatchRef.current = patchJson;
    lastAutoSaveAttemptRef.current = patchJson;
  };
  ...
  useEffect(() => {
    ...
    if (
      ...
      draftPatchJson === lastAutoSaveAttemptRef.current
    ) {
      return;
    }
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        lastAutoSaveAttemptRef.current = draftPatchJson;
        setIsAutoSaving(true);
        const savePromise = onSaveTask(task.id, draftPatch, { lightweight: true });
        ...
        if (!isTaskConflictError(error)) {
          onNotify?.(
            error?.message || "Failed to auto-save task updates.",
            "error",
          );
        }
      } finally {
  ...
  const visibleActivity = useMemo(
    ...
    [activityTab, taskBundle?.activity, taskBundle?.comments],
  );
  ...
  const taskRef = String(
    displayTaskRef(task) || task?.taskKey || task?.id || "",
  )
    .trim()
    .toUpperCase();
  ...
  }, [branchLinks, commitLinks, prLinks]);
  ...
  return (
    <div
      className="fixed inset-0 z-[35] grid place-items-center bg-[rgba(9,30,66,0.34)] p-2 min-[640px]:p-4"
      role="presentation"
      onClick={() => {
    ...
    >
      <aside
        className="grid h-[min(88vh,900px)] w-[min(1160px,calc(100vw-1rem))] grid-rows-[auto_1fr] overflow-hidden rounded-[10px] border border-[#dfe1e6] bg-white shadow-[0_18px_40px_rgba(9,30,66,0.25)] max-[1100px]:h-[calc(100vh-0.5rem)] max-[1100px]:w-[calc(100vw-0.5rem)]"
        role="dialog"
        aria-modal="true"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex flex-wrap items-start justify-between gap-3 px-3 py-[0.75rem] min-[640px]:px-4 min-[640px]:py-[0.9rem]">
          <div>
            <p className="mb-1 text-[0.78rem] tracking-[0.02em] text-[#5e6c84]">{displayTaskRef(task)}</p>
            <h3
              ref={titleEditorRef}
              className={`m-[-0.1rem_-0.2rem_0] cursor-text rounded-md px-[0.2rem] py-[0.1rem] text-[1.18rem] leading-[1.3] outline-none hover:bg-[#f4f5f7] min-[640px]:text-[1.55rem] ${isEditingTitle ? "bg-white shadow-[inset_0_0_0_1px_#b3bac5]" : ""}`}
              role="button"
              tabIndex={0}
        ...
        <div className="grid min-h-0 gap-[0.65rem] overflow-hidden px-2 py-[0.65rem] min-[640px]:px-4 min-[640px]:py-[0.9rem]">
                  ...
                  >
                    ...
                    {devLinksActiveTab === "branches" ? (
                      <div className="grid gap-[0.35rem]">
                        <div className="grid grid-cols-[minmax(0,1fr)_130px_160px] gap-2 px-[0.5rem] text-[0.74rem] font-semibold uppercase tracking-[0.02em] text-[#5e6c84] max-[760px]:hidden">
                          <span>Branch</span>
                          <span>Date</span>
                        ...
                        {repoBucket.branches.map((item) => (
                          <div
                            ...
                            className="grid grid-cols-[minmax(0,1fr)_130px_160px] items-center gap-2 rounded-md border border-[#e5e9f0] bg-[#fbfcff] px-[0.55rem] py-[0.45rem] max-[760px]:grid-cols-1 max-[760px]:items-start"
                          >
                            <a
                              href={item.url || "#"}
                              target="_blank"
                    ...
                    {devLinksActiveTab === "commits" ? (
                      <div className="grid gap-[0.35rem]">
                        <div className="grid grid-cols-[160px_120px_minmax(0,1fr)_130px] gap-2 px-[0.5rem] text-[0.74rem] font-semibold uppercase tracking-[0.02em] text-[#5e6c84] max-[760px]:hidden">
                          <span>Author</span>
                          <span>Commit</span>
                          <span>Message</span>
                          <span>Date</span>
                        ...
                        {repoBucket.commits.map((item) => (
                          <div
                            ...
                            className="grid grid-cols-[160px_120px_minmax(0,1fr)_130px] items-center gap-2 rounded-md border border-[#e5e9f0] bg-[#fbfcff] px-[0.55rem] py-[0.45rem] max-[760px]:grid-cols-1 max-[760px]:items-start"
                          >
                    ...
                    {devLinksActiveTab === "pullRequests" ? (
                      <div className="grid gap-[0.35rem]">
                        <div className="grid grid-cols-[160px_90px_minmax(0,1fr)_100px_130px] gap-2 px-[0.5rem] text-[0.74rem] font-semibold uppercase tracking-[0.02em] text-[#5e6c84] max-[760px]:hidden">
                          <span>Author</span>
                          <span>ID</span>
                          <span>Summary</span>
                          <span>Status</span>
                        ...
                        {repoBucket.pullRequests.map((item) => (
                          <div
                            ...
                            className="grid grid-cols-[160px_90px_minmax(0,1fr)_100px_130px] items-center gap-2 rounded-md border border-[#e5e9f0] bg-[#fbfcff] px-[0.55rem] py-[0.45rem] max-[760px]:grid-cols-1 max-[760px]:items-start"
                          >
                            <span className="truncate text-[0.82rem] text-[#253858]">
                              {getDevItemAuthor(item) || "Unknown author"}
                            </span>
