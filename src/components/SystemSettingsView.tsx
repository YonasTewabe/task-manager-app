...
import { DEFAULT_LABEL_COLOR, normalizeLabelDefinitions } from "../utils/labels.js";
...
export default function SystemSettingsView({
  ...
  const [addStageErrors, setAddStageErrors] = useState<any>({});
  const [addLabelError, setAddLabelError] = useState("");
  const [addTypeError, setAddTypeError] = useState("");
  const [addVersionError, setAddVersionError] = useState("");
  const [addRepoErrors, setAddRepoErrors] = useState<any>({});
  const [addAutomationErrors, setAddAutomationErrors] = useState<any>({});
  const [workflowMoveErrors, setWorkflowMoveErrors] = useState<any>({});
  ...
  const labels = normalizeLabelDefinitions(form.generalRules?.labels);
  const types =
  ...
  const addLabel = () => {
    ...
    const exists = labels.some(
      (label) => String(label.name).toLowerCase() === next.toLowerCase(),
    );
    if (exists) {
      setNewLabel("");
      return;
    ...
    const usedColors = new Set(
      labels
        .map((label) => String(label?.color || "").trim().toLowerCase())
        .filter(Boolean),
    );
    const availablePalette = LABEL_COLOR_PALETTE.filter(
      (color) => !usedColors.has(color),
    );
    const pool = availablePalette.length ? availablePalette : LABEL_COLOR_PALETTE;
    const randomColor = pool[Math.floor(Math.random() * pool.length)];
    setSectionValue("generalRules", "labels", [
      ...labels,
      { name: next, color: randomColor || DEFAULT_LABEL_COLOR },
    ]);
    setNewLabel("");
  };
  const removeLabel = (labelToRemove) => {
    setSectionValue(
      "generalRules",
      "labels",
      labels.filter((label) => label.name !== labelToRemove),
    );
  };
  const addType = () => {
  ...
  const runManualResync = async () => {
    ...
    try {
      ...
      const failedCount = Number(result?.reposFailed || 0);
      if (failedCount > 0) {
        const firstFailure = Array.isArray(result?.failures)
          ? result.failures[0]
          : null;
        const suffix = firstFailure?.repo
          ? ` First failure: ${firstFailure.owner}/${firstFailure.repo} (${firstFailure.error || "unknown error"}).`
          : "";
        onNotify?.(
          `Resync completed with issues: ${result?.linksUpserted || 0} links refreshed, ${failedCount} repos failed.${suffix}`,
          "error",
        );
      } else {
        onNotify?.(
  ...
  return (
    <section className="grid min-w-0 gap-[0.8rem] overflow-x-hidden rounded-lg border border-[#dfe1e6] bg-white p-[0.8rem]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="m-0 min-w-0 flex-1 break-words text-[1.05rem] font-semibold text-[#172b4d]">
      ...
      <div className="flex min-w-0 flex-col gap-3 overflow-x-hidden">
        <div
          className="flex w-full max-w-full flex-nowrap items-center gap-[0.4rem] overflow-x-auto border-b border-[#dfe1e6] pb-1"
          role="tablist"
        ...
        <div className="min-h-8 min-w-0 overflow-x-hidden">
          {activeTab === "board-columns" ? (
            <article
              id="settings-panel-board-columns"
              role="tabpanel"
              aria-labelledby="settings-tab-board-columns"
              className="grid min-w-0 max-w-full gap-[0.55rem] overflow-x-hidden rounded-lg border border-[#dfe3ea] bg-white p-[0.7rem]"
            >
                  ...
                  >
                    ...
                    <div className="flex min-w-0 flex-1 flex-col gap-[0.35rem]">
                      <div className="flex flex-wrap items-center gap-2">
                        <input
                          className="min-w-0 flex-1 rounded border border-[#dfe1e6] px-[0.5rem] py-[0.35rem] font-bold"
                          value={stage.name}
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-3 text-[0.78rem]">
                        ...
                        <label className="flex items-center gap-[0.35rem] text-[0.78rem] text-[#42526e]">
                          <span className="whitespace-nowrap">Roll-up Group</span>
                          <span className="relative inline-flex">
                            <select
                              className="appearance-none rounded border border-[#c9d2e3] bg-white px-[0.5rem] py-[0.3rem] pr-7 text-[0.8rem] text-[#172b4d]"
                    ...
                    {canManage && !BUILTIN_STAGE_KEYS.has(stage.key) ? (
                      <button
                        type="button"
                        className="self-center whitespace-nowrap border border-[#dfe1e6] bg-transparent text-[#42526e] hover:bg-[#f4f5f7]"
                        onClick={() => {
                      ...
                      <span className="hidden w-[4.5rem] flex-shrink-0 min-[640px]:block" />
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 gap-[0.42rem] sm:grid-cols-2 lg:grid-cols-3">
                {labels.length ? (
                  labels.map((label) => (
                    <div
                      key={label.name}
                      className="flex flex-wrap items-center justify-between gap-[0.5rem] rounded-[10px] border border-[#e2e7f1] bg-[#f8fbff] px-[0.55rem] py-[0.45rem]"
                    >
                      <span
                        className="truncate rounded-full border px-[0.52rem] py-[0.2rem] text-[0.75rem] font-semibold text-[#1f3f7f]"
                        style={{
                          backgroundColor: label.color || DEFAULT_LABEL_COLOR,
                          borderColor: label.color || DEFAULT_LABEL_COLOR,
                        }}
                      >
                        {label.name}
                      </span>
          ...
          {activeTab === "workflow" ? (
            ...
            >
              ...
              <p className="max-w-[72ch] break-words text-[0.82rem] leading-[1.35] text-[#6b778c]">
              ...
              <div className="mb-3 flex flex-wrap items-end gap-2">
                <label className="grid w-full gap-[0.25rem] text-[0.82rem] font-semibold text-[#42526e] min-[640px]:w-auto">
                  From stage
                  <span className="relative inline-flex w-full min-w-0 min-[640px]:min-w-[11rem]">
                    <select
              ...
              <div className="flex flex-col gap-2">
                {workflowTransitions.map((transition) => (
                  <div
                    ...
                    className="flex flex-wrap items-start gap-[0.6rem] rounded-lg border border-[#dfe1e6] bg-white px-[0.75rem] py-[0.65rem]"
                  >
                    <div className="flex min-w-0 flex-1 flex-col gap-[0.35rem]">
                      <div className="flex flex-wrap items-center gap-2">
                      ...
                      <div className="grid grid-cols-1 gap-[0.4rem] min-[640px]:grid-cols-2 min-[980px]:grid-cols-3">
                        <label
              ...
              <div className="grid gap-[0.55rem] rounded-[12px] border border-[#dfe3ea] bg-white p-[0.75rem]">
                ...
                {githubRepos.length === 0 ? (
                  ...
                  githubRepos.map((repoItem) => (
                    <div
                      key={repoItem.id}
                    className="flex min-w-0 flex-wrap items-center justify-between gap-3 rounded-[10px] border border-[#e6ebf3] bg-[#f8fbff] px-[0.7rem] py-[0.55rem]"
                    >
                      <div className="grid min-w-0 gap-[0.1rem] text-[0.86rem]">
                        <div>
                      ...
                      <div className="flex w-full flex-wrap justify-end gap-2 min-[640px]:w-auto">
                        <button
                          type="button"
                          disabled={!canManage}
                          className="border border-[#dfe1e6] bg-transparent text-[#42526e] hover:bg-[#f4f5f7]"
          ...
          {activeTab === "users" ? (
            ...
            >
              ...
              <div
                className={`grid grid-cols-1 gap-[0.4rem] min-[640px]:grid-cols-2 min-[980px]:grid-cols-3 ${sortedUsers.length > 6 ? "max-h-[11.5rem] overflow-y-auto pr-1" : ""}`}
              >
              ...
              <div className="grid gap-[0.35rem] rounded-[10px] border border-[#e6ebf3] bg-[#f8fbff] p-[0.65rem]">
                ...
                {sortedUsers.filter(
                  ...
                  <div
                    className={`grid grid-cols-1 gap-[0.4rem] min-[640px]:grid-cols-2 min-[980px]:grid-cols-3 ${sortedUsers.filter((u) => u.role === "member" && memberIds.includes(u.id)).length > 6 ? "max-h-[11.5rem] overflow-y-auto pr-1" : ""}`}
                  >
                    {sortedUsers
                      .filter(
                        (u) => u.role === "member" && memberIds.includes(u.id),
        ...
        >
          ...
          <div className="grid gap-[0.6rem]">
            ...
            <p className="text-[0.78rem] text-[#6b778c]">
              Color is assigned automatically (random, unique in this project).
            </p>
            {addLabelError ? (
